...
import RoomTypeGrid, { ROOM_TYPES } from "@/components/RoomTypeGrid";
...
const BookHotelPage = () => {
  ...
  useEffect(() => {
    if (user?.preferredRoomType) {
      setRoomType(user.preferredRoomType);
    }
  }, [user]);
  useEffect(() => {
  ...
  const roomMultiplier =
    ROOM_TYPES.find((r) => r.key === roomType)?.multiplier || 1;
  const totalPrice = (dynamicPrice ?? hotel?.pricePerNight) * roomMultiplier * quantity;
