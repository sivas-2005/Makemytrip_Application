...
const BookFlightPage = () => {
  ...
  const premiumSeatCount = selectedSeats.filter((s) => {
    const row = Number.parseInt(s.replace(/[^0-9]/g, ""));
    return row <= 2;
  }).length;
  const premiumSurchargeTotal = premiumSeatCount * PREMIUM_SURCHARGE;

  const totalPrice = (dynamicPrice ?? flight?.price) * quantity;
  ...
  const grandTotal =
    totalPrice + totalTaxes + totalOtherServices - totalDiscounts + premiumSurchargeTotal;

  const handlebooking = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedSeats.length !== quantity) {
      alert(`Please select ${quantity} seat${quantity !== 1 ? "s" : ""} to continue.`);
      return;
    }
    try {
    ...
    <DialogContent className="sm:max-w-[600px] bg-white">
      ...
      <div className="grid gap-6 mt-4">
        ...
        <div className="space-y-2">
          ...
          <SeatMap
            totalSeats={
              flight.totalSeats && flight.totalSeats > 0
                ? flight.totalSeats
                : flight.availableSeats + (flight.bookedSeats?.length || 0)
            }
            bookedSeats={flight.bookedSeats || []}
            requiredCount={quantity}
            selectedSeats={selectedSeats}
            onChange={setSelectedSeats}
            premiumSurcharge={PREMIUM_SURCHARGE}
          />
        </div>
          ...
          <div className="space-y-2">
            ...
            {premiumSurchargeTotal > 0 && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  ₹ {premiumSurchargeTotal.toLocaleString()}
                </span>
              </div>
            )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Reviews & Ratings */}
            <ReviewsSection targetType="Flight" targetId={flight?.id} />
          </div>
